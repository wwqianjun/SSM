...
/* 计算配送地址高度  num:总地址个数   i: 每行显示多少个 */
/*
function calcHeight(num, i) {
    var line;
    if (i == 3) {
        line = num % 3 == 0 ? num / 3 : num / 3 + 1;
    }
    if (i == 4) {
        line = num % 4 == 0 ? num / 4 : num / 4 + 1;
    }
    return line * 190;
}*/
function calcHeightThree(addressNum) {
    var line;
    line = addressNum % 3 == 0 ? addressNum / 3 : addressNum / 3 + 1;
    return line * 153 + 60;
};

function calcHeightFour(addressNum) {
    var line;
    line = addressNum % 4 == 0 ? addressNum / 4 : addressNum / 4 + 1;
    return line * 153 + 60;
}

/*function insertUserInfo(addressNum){
    for(var i=0; i<addressNum; i++)
    {
        alert($("#userInfo").find(div).html());
        $("#userInfo").find(div).children().val("");
    }
